...
flowchart TB
    ...
    subgraph DB["数据库 (唯一事实来源)"]
        direction TB
        DB1["化学数据（结构文件、计算结果、元数据）"]
        DB2["会话/消息记录"]
        DB3["用户 LLM 配置"]
        DB4["智能体端和 MCP Server 共享同一数据库实例"]
    end

    subgraph MCPSERVER["MCP 服务端 (lasp_mcp_server)"]
        direction TB
        MCP_DESC["职责：暴露标准化计算工具、调用计算集群、结果入库"]
        MCP_EXT["对外提供：REST 文件上传 + MCP 资源下载"]
        MCP_FLOW["接收 tool call → 提取数据 → 调用计算集群 → 结果入库 → 返回短 ID"]
        subgraph MCP_INT[" "]
            direction LR
...
```mermaid
flowchart LR
    subgraph PATH1["路径 1：内部文件上传/读取"]
        direction LR
        U[用户] -->|"上传/读取"| AGENT1[智能体端]
        AGENT1 -->|"直接写库"| DB1[(数据库)]
        AGENT1 -->|"直接查库"| DB1
    end
```
...
```mermaid
flowchart LR
    subgraph PATH2["路径 2：计算工具调用"]
        direction LR
        AGENT2[智能体端] -->|"MCP 协议"| MCP[MCP Server]
        MCP -->|"结果入库"| DB2[(数据库<br/>同一实例)]
    end
```

```mermaid
flowchart LR
    subgraph PATH3["路径 3：外部客户访问"]
        direction LR
        TP[第三方] -->|"REST 上传"| MCPS[MCP Server]
        TP -->|"MCP 下载"| MCPS
        MCPS -->|"读写"| DB3[(数据库)]
    end
```
