...
erDiagram
    ...
    Conversation ||--o{ ArtifactState : "拥有"
    ...
    FileRecord {
        string description "物理文件元数据，含哈希去重"
    }
```
...
| Conversation → ArtifactState | 一个对话独有拥有的制品；跨对话使用时复制新记录 |
...
CREATE TABLE tool_calls (
    ...
    task_id         VARCHAR(64),                           -- RabbitMQ 任务 ID（异步计算场景）
    created_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
- `task_id` 关联 RabbitMQ 任务，用于异步计算场景的状态追踪和历史审计
...
CREATE TABLE artifact_states (
    ...
    conversation_id     VARCHAR(36) NOT NULL,               -- 所属对话，独有所有权（FK → conversations）
...
| `conversation_id` | 所属对话（独有所有权）。跨对话使用时复制新记录，避免引用计数出错 |
| `source` | 区分内置（builtin）、用户上传（upload）和 MCP 计算产出（computation） |
| `is_deleted` / `deleted_at` | 软删除标记，所属 conversation 删除时标记 |
...
#### artifact_states：对话独有所有权

每个 artifact 归属一个 conversation（`conversation_id` FK），采用**独有所有权**模型：

| 规则 | 说明 |
|------|------|
| 归属 | 创建时绑定 `conversation_id`，不可转移 |
| 跨对话使用 | 复制新记录（新 `string_id` + 新 `conversation_id`），不共享引用 |
| 删除 | 所属 conversation 删除时，标记 `deleted_at` |
| 恢复 | `deleted_at` 宽限期内恢复 conversation 时同步恢复 |

> 不做引用计数的原因：引用计数一旦因 bug 出现偏差，会导致数据残留或误删，排查成本高。独有所有权逻辑简单、无状态不一致风险。

#### 物理文件：定期清理

物理文件不依赖引用计数，由文件管理服务器的定时任务统一清理：

```text
清理条件：
  artifact 所属 conversation 满足以下任一条件：
    - conversation.deleted_at < NOW() - 30 天（软删除宽限期已过）
    - conversation.updated_at < NOW() - 7 天 且 status = 'archived'
  → 删除物理文件 + file_records 记录
  → artifact_states 保留，file_server_ids 中对应 ID 标记失效
```
...
| ArtifactState 归属 | 归属 `conversation_id`（独有所有权） | 跨对话复制，避免引用计数不一致风险 |
| 短 ID 机制 | 带类型前缀的随机串 |  |
