...
| `entities/`       | 化学数据实体模型（位于 `utils/entities/`）。保留 `read()` / `get_summary()` / `export_metadata()` / `read_file()` 等业务方法，底层数据访问切换为直接数据库查询 |
...
该模块不依赖 MCP，仅弱依赖 `utils/entities`（`ArtifactSummary`），路径通过 `entities/__init__.py` re-export 保持稳定。
...
- **异步任务队列**：MCP 服务端对耗时较长的本地计算脚本采用 RabbitMQ 队列进行异步调度，避免 MCP 请求-响应模式阻塞连接。`tool_calls` 表已预留 `task_id` 字段对接队列系统。
