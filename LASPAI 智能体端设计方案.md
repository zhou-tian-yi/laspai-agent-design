...
│   ┌────────────────────────────────────────┼───────────────────────────┐│
│   │                core/                   │  直连数据库                  ││
│   │  实体模型 / 配置 / 日志 / context       │  (数据存取，不走 MCP)         ││
│   └────────────────────────────────────────┼───────────────────────────┘│
│                                            │                             │
└──────────────────────────────┬─────────────┼─────────────────────────────┘
                               │ 直连 DB     │ MCP 协议
...
│                              │  │                                        │
│  化学结构 / 计算结果           │  │  接收 tool call → 数据库提取数据          │
│  用户数据 / 会话记录           │◀─│  → 调用计算集群 → 结果入库 → 返回 ID       │
│  所有持久化数据               │  │                                        │
...
```text
START → setup → LLM → router ──→ 子智能体 A → post_process ──→ LLM（循环）
                           ├──→ 子智能体 B → post_process ──→
                           ├──→ 原生工具    → LLM（直接循环）
                           └──→ END（无 tool call 时直接回复）
```
